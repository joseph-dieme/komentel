...
export const KomentelProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // 1. Fallback Local Soccer Simulation Loop
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setMatches(prevMatches => {
        ...
        const nextMatches = prevMatches.map(m => {
          if (m.status !== "EN DIRECT" || m.minute === undefined || m.sport !== 'FOOTBALL') return m;

          updated = true;
          let nextMinute = m.minute + 1;
          let nextDetail = m.detail;
          let nextStatus: "EN DIRECT" | "Terminé" | "À venir" = m.status;
          let nextScore = m.score;

          // Game progression logic
          if (m.half === 1) {
            if (nextMinute > 45) {
              return { ...m, half: 2 as 1 | 2, minute: 46, detail: "H2 46'" };
            } else {
              nextDetail = `H1 ${nextMinute}'`;
            }
          } else if (m.half === 2) {
            if (nextMinute > 90) {
              if (Math.random() < 0.25) {
                ...
                nextDetail = `90+${nextMinute - 90}'`;
              }
            } else {
              nextDetail = `H2 ${nextMinute}'`;
            }
          }

          // Goal simulation logic (4% chance of a goal per minute tick)
          if (nextStatus === "EN DIRECT" && Math.random() < 0.04) {
            ...
            nextScore = `${homeScore} - ${awayScore}`;

            // Push a dynamic global notification
            const scoringTeam = homeScores ? m.homeTeam : m.awayTeam;
            const goalText = `⚽ BUT ! ${scoringTeam.flag} ${scoringTeam.name} marque ! Nouveau score : ${m.homeTeam.name} ${nextScore} ${m.awayTeam.name} (${nextDetail})`;
  ...
  // 1b. Simulation Loop for Basketball and Lutte Sénégalaise
  useEffect(() => {
    const interval = setInterval(() => {
      setMatches(prevMatches => {
        let updated = false;
        const nextMatches = prevMatches.map(m => {
          if (m.status !== "EN DIRECT") return m;

          // Basketball Score & Time Simulation
          if (m.sport === 'BASKETBALL') {
            updated = true;
            let newHomeScore = m.homeScore || 78;
            let newAwayScore = m.awayScore || 76;
            if (Math.random() > 0.5) {
              const incrementHome = Math.random() > 0.5;
              const pts = Math.random() > 0.6 ? 3 : 2;
              if (incrementHome) newHomeScore += pts;
              else newAwayScore += pts;
            }

            let nextTime = m.timeRemaining || "8'32\"";
            let nextQuarter = m.quarter || 4;
            let nextStatus: 'EN DIRECT' | 'Terminé' | 'À venir' = m.status;
            let nextDetail = m.detail;

            const timeParts = nextTime.split("'");
            let min = parseInt(timeParts[0]) || 0;
            let sec = parseInt(timeParts[1]?.replace('"', '')) || 0;
            ...
            if (min < 0) {
              ...
              nextTime = `${min}'${sec.toString().padStart(2, '0')}"`;
              nextDetail = `Q${nextQuarter} ${nextTime}`;
            }
          ...
          // Lutte Sénégalaise Live Combat Stage Simulation
          if (m.sport === 'LUTTE') {
            updated = true;
            const currentStage = m.wrestlingStage !== undefined ? m.wrestlingStage : 0;
            ...
            if (Math.random() < 0.3) {
              ...
              if (nextStage === 1) {
                nextDetail = "Face à face";
                nextScore = "Préparation";
              } else if (nextStage === 2) {
                nextDetail = "Combat au corps à corps";
                nextScore = "0 - 0";
              } else if (nextStage === 3) {
                const winnerIsModou = Math.random() > 0.5;
                nextDetail = winnerIsModou ? "Victoire Modou Lô" : "Victoire Balla Gaye 2";
                nextScore = "Chute";
                nextStatus = "Terminé";
              }
            }
